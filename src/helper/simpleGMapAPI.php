...
class simpleGMapAPI {
    ...
    {
        if (!$height) { $height = $this->mapHeight; }
    ...
    function printGMapsJS($https = false)
    {
        ...
        if($https)
        {
            $jsCode .= "\n<script src=\"//maps.google.com/maps/api/js?key=".$siteconfig->MapsAPIKey."\" type=\"text/javascript\"></script>\n";
     ...
     */
    function addMarker($lat, $lng, $tooltip="", $info="", $iconURL="", $clickable=true, $directlink = "")
    {
        ...
        $this->mapMarkers[$count]['directlink'] = $directlink;
     ...
     *               @description  Add's a Marker to be displayed on the Google Map using latitude/longitude
     */
    function addMarkerByAddress($address, $tooltip="", $info="", $iconURL="", $clickable=true, $directlink = "",$output = false)
    {
    ...
    function showMap($zoomToBounds = true)
    {
        ...
        $mapSytle = ($this->mapStyle)?$this->mapStyle:'[]';
        ...
        $mapCode .= "\n<script type=\"text/javascript\">\n".
            ...
            "var latlng = new google.maps.LatLng(".str_replace(',','.',$this->centerLat).", ".str_replace(',','.',$this->centerLng).");\n".
            "var options = {\n".
            "\tzoom: ".$this->zoomLevel.",\n".
            "\tcenter: latlng,\n".
            "\tmapTypeId: google.maps.MapTypeId.".$this->mapType.",\n";
        if ( $this->mapBackgroundColor != "" ) { $mapCode .= "backgroundColor: '".$this->mapBackgroundColor."',\n"; }
        $mapCode .= "\tdisableDefaultUI: ".$_disableDefaultUI.",\n".
        ...
        {
            $mapCode .= "\tgoogle.maps.event.addListener(map, 'click', function() { if (currentInfoWindow != null) { currentInfoWindow.close(); } });\n";
        }
        ...
        for ( $count = 0; $count < sizeof($this->mapMarkers); $count++ )
        {
            ...
            $mapCode .= "\tvar markerLatLng = new google.maps.LatLng(".str_replace(',','.',$this->mapMarkers[$count]['lat']).", ".str_replace(',','.',$this->mapMarkers[$count]['lng']).");\n".
                "\tvar marker$count = new google.maps.Marker({\n".
                "\t position: markerLatLng,\n".
                "\t title: '".$this->mapMarkers[$count]['tooltip']."',\n";
            if ( $this->mapMarkers[$count]['iconURL'] != "" )
            {
                $mapCode .= "\t icon: '".$this->mapMarkers[$count]['iconURL']."',\n";
            }
            if ( $this->mapMarkers[$count]['clickable'] == false )
            {
                $mapCode .= "\t clickable: false,\n";
            }
            if($this->mapMarkers[$count]['directlink'] != "")
            {
                $mapCode .= "\t url:'".$this->mapMarkers[$count]['directlink']."',\n";
            }
            $mapCode .= "\t map: map\n".
                "\t});\n";
            ...
            {
                // create InfoWindow
                $mapCode .= "\tvar infowindowM$count = new google.maps.InfoWindow({\n".
                    "\t content: '".$this->mapMarkers[$count]['info']."'\n".

                    "\t});\n";
                // add an event to the marker
                $mapCode .= "\tgoogle.maps.event.addListener (marker$count, '$_infowindowtrigger', function() {\n";
                // infoWindowBehaviour
                if ( ($this->infoWindowBehaviour == 'SINGLE') OR ($this->infoWindowBehaviour == 'SINGLE_CLOSE_ON_MAPCLICK') )
                {
                    $mapCode .= "\t if (currentInfoWindow != null) { currentInfoWindow.close(); } \n";
                }
                $mapCode .= "\t infowindowM$count.open(map, marker$count);\n".
            ...
            if($this->mapMarkers[$count]['directlink'] != "")
            {
                ...
                $mapCode .= "\tgoogle.maps.event.addListener (marker$count, 'click', function() {\n";
                $mapCode .= "\t window.location.href = this.url;\n";
                $mapCode .= "\t});\n";
            }
            $mapCode .= "\tbounds.extend(markerLatLng);\n\n";
        }
        ...
        for ( $count = 0; $count < sizeof($this->mapCircles); $count++ )
        {
            ...
            $mapCode .= "\tvar circleLatLng = new google.maps.LatLng(".$this->mapCircles[$count]['lat'].", ".$this->mapCircles[$count]['lng'].");\n".
                "\tvar circle$count = new google.maps.Circle({\n".
                "\t center: circleLatLng,\n".
                "\t radius: ".$this->mapCircles[$count]['rad'].",\n";
            // check if there are options set for the circle
            if ( $this->mapCircles[$count]['fillColor'] != "" )     { $mapCode .= "\t fillColor: '".$this->mapCircles[$count]['fillColor']."',\n"; }
            if ( $this->mapCircles[$count]['fillOpacity'] != "" )   { $mapCode .= "\t fillOpacity: ".$this->mapCircles[$count]['fillOpacity'].",\n"; }
            if ( $this->mapCircles[$count]['strokeColor'] != "" )   { $mapCode .= "\t strokeColor: '".$this->mapCircles[$count]['strokeColor']."',\n"; }
            if ( $this->mapCircles[$count]['strokeOpacity'] != "" ) { $mapCode .= "\t strokeOpacity: ".$this->mapCircles[$count]['strokeOpacity'].",\n"; }
            if ( $this->mapCircles[$count]['strokeWeight'] != "" )  { $mapCode .= "\t strokeWeight: ".$this->mapCircles[$count]['strokeWeight'].",\n"; }
            if ( $this->mapCircles[$count]['clickable'] == false )  { $mapCode .= "\t clickable: false,\n"; }
            $mapCode .= "\t map: map\n".
                "\t});\n";
            ...
            {
                // create InfoWindow
                $mapCode .= "\tvar infowindowC$count = new google.maps.InfoWindow({\n".
                    "\t content: '".$this->mapCircles[$count]['info']."',\n".
                    "\t position: circleLatLng,\n".
                    "\t});\n";
                // add an event to the marker
                $mapCode .= "\tgoogle.maps.event.addListener (circle$count, '$_infowindowtrigger', function() {\n";
                // infoWindowBehaviour
                if ( ($this->infoWindowBehaviour == 'SINGLE') OR ($this->infoWindowBehaviour == 'SINGLE_CLOSE_ON_MAPCLICK') )
                {
                    $mapCode .= "\t if (currentInfoWindow != null) { currentInfoWindow.close(); } \n";
                }
                $mapCode .= "\t var tmplat1 = circle$count.getCenter().lat()+(circle$count.getBounds().getNorthEast().lat() - circle$count.getCenter().lat())/2;\n";
                $mapCode .= "\t var tmplng1 = circle$count.getCenter().lng()+(circle$count.getBounds().getNorthEast().lng() - circle$count.getCenter().lng())/2;\n";
                $mapCode .= "\t var newpos = new google.maps.LatLng(tmplat1, tmplng1);\n";
                $mapCode .= "\t infowindowC$count.open(map);\n".
                    "\t infowindowC$count.setPosition(newpos);\n".
                    "\t currentInfoWindow = infowindowC$count;\n".
                    "\t});\n";
            }

            $mapCode .= "\tbounds.extend(circle$count.getBounds().getNorthEast());\n";
            $mapCode .= "\tbounds.extend(circle$count.getBounds().getSouthWest());\n\n";
        }
        ...
        for ( $count = 0; $count < sizeof($this->mapRectangles); $count++ )
        {
            ...
            $mapCode .= "\tvar rectangleSW = new google.maps.LatLng(".$this->mapRectangles[$count]['lat1'].", ".$this->mapRectangles[$count]['lng1'].");\n".
            ...
            if ( $this->mapRectangles[$count]['fillColor'] != "" )     { $mapCode .= "\t fillColor: '".$this->mapRectangles[$count]['fillColor']."',\n"; }
            if ( $this->mapRectangles[$count]['fillOpacity'] != "" )   { $mapCode .= "\t fillOpacity: ".$this->mapRectangles[$count]['fillOpacity'].",\n"; }
            if ( $this->mapRectangles[$count]['strokeColor'] != "" )   { $mapCode .= "\t strokeColor: '".$this->mapRectangles[$count]['strokeColor']."',\n"; }
            if ( $this->mapRectangles[$count]['strokeOpacity'] != "" ) { $mapCode .= "\t strokeOpacity: ".$this->mapRectangles[$count]['strokeOpacity'].",\n"; }
            if ( $this->mapRectangles[$count]['strokeWeight'] != "" )  { $mapCode .= "\t strokeWeight: ".$this->mapRectangles[$count]['strokeWeight'].",\n"; }
            if ( $this->mapRectangles[$count]['clickable'] == false )  { $mapCode .= "\t clickable: false,\n"; }
            $mapCode .= "\t map: map\n".
                "\t});\n";
            ...
            {
                // create InfoWindow
                $mapCode .= "\tvar infowindowR$count = new google.maps.InfoWindow({\n".
                    "\t content: '".$this->mapRectangles[$count]['info']."',\n".
                    "\t position: rectangleNE,\n".
                    "\t});\n";
                // add an event to the marker
                $mapCode .= "\tgoogle.maps.event.addListener (rectangle$count, '$_infowindowtrigger', function() {\n";
                // infoWindowBehaviour
                if ( ($this->infoWindowBehaviour == 'SINGLE') OR ($this->infoWindowBehaviour == 'SINGLE_CLOSE_ON_MAPCLICK') )
                {
                    $mapCode .= "\t if (currentInfoWindow != null) { currentInfoWindow.close(); } \n";
                }
                $mapCode .= "\t infowindowR$count.open(map);\n".
                    "\t currentInfoWindow = infowindowR$count;\n".
                    "\t});\n";
            }

            $mapCode .= "\tbounds.extend(rectangleNE);\n";
            $mapCode .= "\tbounds.extend(rectangleSW);\n\n";
        }

        if ( $zoomToBounds )
        {
            $mapCode .= "\tmap.fitBounds(bounds);\n";
        }
        $mapCode .= "}\n\n".
